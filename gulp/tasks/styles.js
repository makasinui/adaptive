...
const sassBuild = () => (
    gulp.src([`${config.src.sass}/main.sass`, `${config.src.sass}/loans.sass`], { sourcemaps: config.isDev })
        .pipe(sassGlob())
        .pipe(sass({
            includePaths: ['./node_modules'],
        }))
        .pipe(gulpif(config.isProd, gcmq()))
        .pipe(gulpif(config.isProd, autoprefixer(["last 15 versions", "> 1%", "ie 9", "ie 10", "ie 11"])))
        .pipe(rename({
            suffix: '.min',
        }))
